...
class CNFGeneratorBase(object):
    ...
    def _to_json(self, n, m, graph_map, edge_feature):
        # return [[n, m], list(((graph_map[0, :] + 1) * edge_feature).astype(int)), list(graph_map[1, :] + 1), label]
        return [[n, m], list(((graph_map[0, :] + 1) * edge_feature).astype(int)), list(graph_map[1, :] + 1)]
...
class UniformCNFGenerator(CNFGeneratorBase):
    ...
    def generate(self):
        ...
        m = int(n * alpha)
        clause_len = np.random.randint(self._min_k, min(self._max_k, n-1) + 1)
        clause_length = [clause_len for _ in range(m)]
    ...
    def generate_complete(self):
        n = np.random.randint(self._min_n, self._max_n + 1)
        alpha = np.random.uniform(self._alpha, self._alpha + 2*self._alpha_inc)
        alpha = 3.4 if alpha < self._alpha + self._alpha_inc else 5.25
        m = int(n * alpha)
        max_trial = 10
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_a', action='store', dest='max_a', type=float, default=6)
