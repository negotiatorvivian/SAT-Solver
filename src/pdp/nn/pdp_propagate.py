...
class NeuralMessagePasser(nn.Module):
    ...
    def __init__(self, device, edge_dimension, decimator_dimension, meta_data_dimension, hidden_dimension,
        ...
        self._variable_aggregator = util.TransformAggregator(device, decimator_dimension + edge_dimension + meta_data_dimension,
        ...
        self._function_aggregator = util.TransformAggregator(device, decimator_dimension + edge_dimension + meta_data_dimension,
                                                             hidden_dimension, mem_hidden_dimension,
                                                             mem_agg_hidden_dimension, agg_hidden_dimension, edge_dimension,
                                                             include_self_message = False)
    ...
    def forward(self, init_state, decimator_state, sat_problem, is_training, active_mask = None):
        ...
        function_state = mask * self._variable_aggregator(decimator_variable_state, decimator_function_state, sat_problem._edge_feature,
                                                          variable_mask, variable_mask_transpose, edge_mask) + (1 - mask) * function_state
        ...
        ## functions --> variables
        variable_state = mask * self._function_aggregator(decimator_function_state, decimator_variable_state, sat_problem._edge_feature,
                                                          function_mask, function_mask_transpose, edge_mask) + (1 - mask) * variable_state
