...
class MessageAggregator(nn.Module):
    ...
    def __init__(self, device, input_dimension, output_dimension, mem_hidden_dimension,
        ...
        self.agg_hidden_dimension_1 = agg_hidden_dimension - 50
        self.mem_hidden_dimension_1 = mem_hidden_dimension - 50
        ...
        if mem_hidden_dimension > 0 and mem_agg_hidden_dimension > 0:
            ...
            # self._W1_m1 = nn.Linear(
            #     mem_hidden_dimension, self.mem_hidden_dimension_1, bias=True)  # .to(self._device)
            ...
            # self._module_list.append(self._W1_m1)
        ...
        if agg_hidden_dimension > 0 and mem_agg_hidden_dimension > 0:
            ...
            # self._W1_a1 = nn.Linear(
            #     agg_hidden_dimension, self.agg_hidden_dimension_1, bias=True)  # .to(self._device)

            self._W2_a = nn.Linear(
                agg_hidden_dimension, output_dimension, bias=False)  # .to(self._device)

            self._module_list.append(self._W1_a)
            # self._module_list.append(self._W1_a1)
    ...
    def forward(self, state, feature, mask, mask_transpose, edge_mask=None):
        ...
        if not self._include_self_message:
            if mask_transpose.dtype == torch.float16 or aggregated_state.dtype == torch.float16:
                ...
                aggregated_state = aggregated_state.to(dtype = torch.float)
                aggregated_state = torch.mm(mask_transpose, aggregated_state).to(dtype = torch.half)
            else:
        ...
        if feature is not None:
            if aggregated_state.dtype == torch.float16 or feature.dtype == torch.float16:
                feature = feature.to(dtype = torch.float)
                aggregated_state = aggregated_state.to(dtype = torch.float)
                aggregated_state = torch.cat((aggregated_state, feature), 1).to(dtype = torch.half)
            else:
        ...
        if self._agg_hidden_dimension > 0 and self._mem_agg_hidden_dimension > 0:
            # aggregated_state = F.relu(self._W2_a(F.logsigmoid(self._W1_a(aggregated_state))))
...
class SatLossEvaluator(nn.Module):
    ...
    def forward(self, variable_prediction, label, graph_map, batch_variable_map, 
        ...
        # print(torch.tensor([max_coeff], device=self._device).dtype, global_step.pow(self._alpha).dtype)

        if torch.tensor([max_coeff], device=self._device).dtype == torch.float and global_step.pow(self._alpha).dtype ==torch.float16:
            coeff = torch.min(global_step.pow(self._alpha), torch.tensor([max_coeff], device=self._device, dtype = torch.float16))
        else:
...
class SatCNFEvaluator(nn.Module):
    ...
    def forward(self, variable_prediction, graph_map, batch_variable_map, 
        ...
        # print('\ngraph_map', [int(v) for v in graph_map[1].cpu().numpy()])
        ...
        # print('\nclause_values', [int(v) for v in clause_values.cpu().numpy()])
