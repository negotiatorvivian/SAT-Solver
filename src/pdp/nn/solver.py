...
class SATProblem(object):
    ...
    def _peel(self):
        ...
        while True:
            ...
            signed_degree_delta = torch.mm(signed_vf_map.to(torch.float), single_functions.to(torch.float)) * self._active_variables
...
class PropagatorDecimatorSolverBase(nn.Module):
    ...
    def forward(self, init_state,
        ...
        meta_data, is_training=True, iteration_num=10, check_termination=None, simplify=True, batch_replication=10):
    ...
    def _deduplicate(self, prediction, propagator_state, decimator_state, sat_problem):
        ...
        for x in propagator_state:
            new_propagator_state += ((flag * x).view(sat_problem._batch_replication, int(sat_problem._edge_num / sat_problem._batch_replication), -1).sum(dim=0),)

        new_decimator_state = ()
        for x in decimator_state:
            new_decimator_state += ((flag * x).view(sat_problem._batch_replication, int(sat_problem._edge_num / sat_problem._batch_replication), -1).sum(dim=0),)
