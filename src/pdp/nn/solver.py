...
class SATProblem(object):
    ...
    def setup_problem(self, data_batch, batch_replication):
        ...
        if batch_replication > 1:
            ...
            self._graph_map, self._batch_variable_map, self._batch_function_map, self._edge_feature, \
            self._meta_data, _ = self._replicate_batch(data_batch, batch_replication)
        else:
            self._graph_map, self._batch_variable_map, self._batch_function_map, self._edge_feature, \
            self._meta_data, _, variable_num, function_num = data_batch

        self._variable_num = self._batch_variable_map.size()[0]
        self._function_num = self._batch_function_map.size()[0]
        self._edge_num = self._graph_map.size()[1]/100
    ...
    def _peel(self):
        ...
        signed_variable_degree = torch.mm(signed_vf_map.to(torch.float), self._active_functions.to(torch.float))
        ...
        while True:
            ...
            signed_degree_delta = torch.mm(signed_vf_map.to(torch.float),
                                           single_functions.to(torch.float)) * self._active_variables
...
class PropagatorDecimatorSolverBase(nn.Module):
    ...
    def forward(self, init_state, graph_map, batch_variable_map, batch_function_map, edge_feature, edge_embedding,
                meta_data, is_training = True, variable_num = None, function_num = None, iteration_num = 10,
                check_termination = None,
                simplify = True, batch_replication = 10):
        ...
        sat_problem = SATProblem((graph_map, batch_variable_map, batch_function_map, edge_feature, meta_data, None,
                                  variable_num, function_num), self._device, batch_replication)
    ...
    def _deduplicate(self, prediction, propagator_state, decimator_state, sat_problem):
        """De-duplicates the current batch (to neutralize the batch replication) by finding the replica with minimum
        energy for each problem instance. """
        ...
        for x in propagator_state:
            new_propagator_state += (
            (flag * x).view(sat_problem._batch_replication, int(sat_problem._edge_num / sat_problem._batch_replication),
                            -1).sum(dim = 0),)
    ...
    def get_init_state(self, graph_map, batch_variable_map, batch_function_map, edge_feature, graph_feat,
                       variable_num, function_num, randomized, batch_replication = 1):
        ...
        if self._propagator is None:
            ...
            init_propagator_state = self._propagator.get_init_state(graph_map, batch_variable_map, batch_function_map,
                                                                    edge_feature, graph_feat, variable_num, function_num,
...
class NeuralSurveyPropagatorSolver(PropagatorDecimatorSolverBase):
    ...
    def __init__(self, device, name, edge_dimension, meta_data_dimension,
        ...
        super(NeuralSurveyPropagatorSolver, self).__init__(
            ...
            decimator = pdp_decimate.NeuralDecimator(device, (3, 2), meta_data_dimension,
                                                     decimator_dimension, mem_hidden_dimension,
                                                     mem_agg_hidden_dimension, agg_hidden_dimension, edge_dimension,
                                                     dropout),
            predictor = pdp_predict.NeuralPredictor(device, decimator_dimension, prediction_dimension,
    ...
    def __init__(self, device, name, edge_dimension, meta_data_dimension,
        ...
        super(NeuralSequentialDecimatorSolver, self).__init__(
            ...
            propagator = pdp_propagate.NeuralMessagePasser(device, edge_dimension, decimator_dimension,
                                                           ...
                                                           mem_agg_hidden_dimension, agg_hidden_dimension, dropout),
            decimator = pdp_decimate.SequentialDecimator(device, message_dimension = (3, 1),
                                                         scorer = pdp_predict.NeuralPredictor(device,
